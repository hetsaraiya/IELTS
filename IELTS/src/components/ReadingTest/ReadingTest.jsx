...
///////////// MCQ //////////////////
  ...
  const mcqDesc = "Choose TWO correct letters";
...
const [summaryValues, setSummaryValues] = useState(Array(summary.length).fill(''));
          ...
          <React.Fragment key={partIndex}>
            ...
            {partIndex < parts.length - 1 && (
              ...
              <input
                ...
                onChange={(e) => {handleSummaryChange(index, e.target.value); handleInputChange(item.qno);}}
...
///////////// SUMMARY COMPLETION END//////////////////

///////////// TRUE-FALSE //////////////////
const trueFalseOptions = ["True", "False", "Not given"];

const trueFalse = [
  // {
  //   no: 11,
  //   ques: "Question number eleven",
  //   answer: "True",
  // },
  // {
  //   no: 12,
  //   ques: "Question number twelve",
  //   answer: "False",
  // },
  // {
  //   no: 13,
  //   ques: "Question number thirteen",
  //   answer: "Not given",
  // },
];

const handleTrueFalse = (e, questionIndex) => {
  const { value } = e.target;
  setTrueFalseValues((prevValues) => ({
    ...prevValues,
    [questionIndex]: value,
  }));
};

const [trueFalseValues, setTrueFalseValues] = useState({});

///////////// TRUE-FALSE END//////////////////

///////////// YES-NO //////////////////
const yesNoOptions = ["Yes", "No", "Not given"];

const yesNo = [
  // {
  //   no: 14,
  //   ques: "Question number fourteen",
  //   answer: "Yes",
  // },
  // {
  //   no: 15,
  //   ques: "Question number fifteen",
  //   answer: "No",
  // },
];

const handleYesNo = (e, questionIndex) => {
  const { value } = e.target;
  setYesNoValues((prevValues) => ({
    ...prevValues,
    [questionIndex]: value,
  }));
};
...
const headingOptions = ["1", "2", "3", "4", "5"];
const headingDesc =
...
const headingsList = [
  "1 Heading number one",
  "2 Heading number two",
  "3 Heading number three",
  "4 Heading number four",
  "5 Heading number five",
];

const handleHeading = (e, questionIndex) => {
  const { value } = e.target;
  setHeadingValues((prevValues) => ({
    ...prevValues,
    [questionIndex]: value,
  }));
};
...
///////////// MATCHING HEADINGS OPTIONS END//////////////////

const diagramDesc =
    "Label the Diagram and write the correct answer next to it's number";
  const diagram = [];
  ...
  const handleSubmit = (e) => {
    ...
    if (sortedQuestions["Matching Information"]?.length > 0) {
      sortedQuestions["Matching Information"].forEach((question, index) => {
        const selectedOption = document.getElementById(`matching-${index}`).value;
        const correctAnswer = question.answer;
    ...
    if (sortedQuestions["Summary Completion"]?.length > 0) {
      sortedQuestions["Summary Completion"].forEach((question, index) => {
        const selectedAnswer = document.getElementById(`${summaryValues[index]}`).value;
        const correctAnswer = question.answer;
    ...
    if (sortedQuestions["True False"]?.length > 0) {
      sortedQuestions["True False"].forEach((question, index) => {
        const selectedOption = document.getElementById(`trueFalse-${index}`).value;
        const correctAnswer = question.answer;
    ...
    if (sortedQuestions["Yes No"]?.length > 0) {
      sortedQuestions["Yes No"].forEach((question, index) => {
        const selectedOption = document.getElementById(`yesNo-${index}`).value;
        const correctAnswer = question.answer;
    ...
    if (sortedQuestions["Matching Heading"]?.length > 0) {
      sortedQuestions["Matching Heading"].forEach((question, index) => {
        const selectedOption = document.getElementById(`heading-${index}`).value;
        const correctAnswer = question.answer;
   ...
   useEffect(() => {
    const totalQuestions = (sortedQuestions["Sentence Completion MCQ"]?.length ?? 0) +
    ...
    return (
      <div className="numbers flex flex-row justify-center gap-y-5 flex-wrap mt-[17px] gap-x-[15px]">
        {attemptedQuestions.map((attempted, index) => (
          <div className={`md:w-[38px] md:h-[38px] w-[28px] h-[28px] rounded-full flex justify-center items-center ${attempted ? "bg-[#FF467A]" : "bg-[#E4E4E4]"}`}>
            <p className={`md:text-[16px] text-[12px] font-medium text-${attempted ? "white" : "#303030"}`}>{index +1}</p>
          </div>
        ))} 
      </div>
    );
...
//     const paragraphs = Array.from(tempElement.querySelectorAll('p'));
...
const formatContent = (content) => {
  ...
  return (
    <div className='font-prompt relative'>
       ...
       <form onSubmit={handleSubmit} className="w-[100%]  flex  flex-col  gap-x-[35px] mt-[80px] ">
       {deleteOpen && <div className="fixed z-40 top-0 left-0 right-0 bottom-0 bg-black bg-opacity-20 flex justify-center">
            <div className="md:w-[450px] w-[300px] h-[276px] bg-white mt-10 flex flex-col justify-center items-center">
              <p className="md:text-[18px] text-[14px] font-medium text-[#2B2D42] ">Are you sure you want to SUBMIT?</p>
              <button type="submit" className="w-[200px] h-[40px] border border-[#2B2D42] text-[#2B2D42] mt-[46px] text-[16px] font-medium ">Submit</button>
          ...
          {exitOpen && <div className="fixed z-40 top-0 left-0 right-0 bottom-0 bg-black bg-opacity-20 flex justify-center">
            <div className="md:w-[450px] w-[300px] h-[276px] bg-white mt-10 flex flex-col justify-center items-center">
              <p className="md:text-[18px] text-[14px] font-medium text-[#2B2D42] ">Are you sure you want to EXIT?</p>
              <Link to="/readingtestcards"  className="cursor-pointer flex justify-center items-center w-[200px] h-[40px] border border-[#2B2D42] text-[#2B2D42] mt-[46px] text-[16px] font-medium ">Exit</Link>
              <div onClick={handleRefresh} className="w-[200px] h-[40px] border border-[#2B2D42] text-[#2B2D42] flex cursor-pointer justify-center items-center mt-[10px] text-[16px] font-medium ">Retake the test</div>
              <div onClick={()=>setExitOpen(false)} className="w-[200px] h-[40px] bg-[#2B2D42] cursor-pointer text-white flex justify-center items-center mt-[10px] text-[16px] font-medium ">Cancel</div>
            </div>
          </div>}
         ...
         <div className='flex w-full h-full lg:flex-row flex-col lg:px-20 gap-y-5 gap-x-5 '>
           <div className='content lg:w-[50%] w-full lg:h-[70vh] h-[400px] overflow-y-scroll pt-10 bg-[#F6F8F9] pb-[200px]'>
            
            {/* <p className='text-[26px] font-medium text-[#303030] text-center'>Title of Article</p>
            <div className='w-[200px] h-[200px] mx-auto mt-[16px] '>
              <img src='/assets/readingImg.png' className='w-full h-full'/>
            </div> */}
            <div className='px-10 mt-10 text-[#303030]'>
              <p className='text-[26px] font-medium '>Content</p>
              <p className='text-[18px] font-normal mt-[16px] 'dangerouslySetInnerHTML={{ __html: formatContent(testContent) }}></p>
            </div>
           </div>

           <div className='content lg:w-[50%] w-full lg:h-[70vh] h-[400px] overflow-y-scroll bg-[#F6F8F9] pt-10 pb-[200px]'>
           {sortedQuestions["Sentence Completion MCQ"]?.length > 0 && (
                    ...
                    <div className="mt-5 md:ml-16 ml-10">
                      ...
                      <div className="flex mt-[11px] font-normal text-[18px]  ">
                        ...
                        <input
                          ...
                          onChange={(e) => {handleChange(e); handleInputChange(question.qno);}}
                        />{" "}
                    ...
                    <div className="mt-5 md:ml-16 ml-10">
                      ...
                      <div className="flex mt-[11px] font-normal text-[18px]  ">
                        ...
                        <input
                          ...
                          value={question.option1}
                          onChange={(e) => {handleChange2(e); handleInputChange(question.qno);}}
                        />{" "}
                        <p>{question.option1}</p>
                      </div>
                      <div className="flex mt-[6px] font-normal text-[18px] ">
                    ...
                    <div className="mt-[6px] md:ml-16 ml-10">
                      <div className="flex text-[18px] font-normal  ">
                        ...
                        <select
                          ...
                          onChange={(e) => {handleMatchingChange(e, index); handleInputChange(question.qno);}}
            ...
            {sortedQuestions["True False"]?.length > 0 && (
              <div className="sentence w-full bg-[#F6F8F9] h-fit py-10 mt-[18px] ">
                ...
                <p className=" text-[18px] text-[#303030] pt-5 md:pl-12 pl-8">
                TRUE - if the statement agrees with the information
                </p>
                <p className=" text-[18px] text-[#303030] pt-1 md:pl-12 pl-8">
                FALSE - if the statement contradicts the information
                </p>
                <p className=" text-[18px] text-[#303030] pt-1 md:pl-12 pl-8 mb-[18px]">
                    ...
                    <div className="mt-[6px] md:ml-16 ml-10">
                      <div className="flex text-[18px] font-normal  ">
                        ...
                        <select
                          name="trueFalse"
                          id={`trueFalse-${index}`}
                          className=" w-[64px] h-[20px] border rounded-[5px] flex flex-col justify-center text-[12px] "
                          onChange={(e) => {handleTrueFalse(e, index); handleInputChange(question.qno);}}
                          value={trueFalseValues[index] || ""}
                        >
                           <option value={null}>Select</option> 
                          {trueFalseOptions.map((options, index) => {
                            return <option key={options} value={options}>{options}</option>;
                          })}
                        </select>{" "}
                    ...
                    <div className="mt-[6px] md:ml-16 ml-10">
                      <div className="flex text-[18px] font-normal  ">
                        ...
                        <select
                          ...
                          id={`yesNo-${index}`}
                          className=" w-[64px] h-[20px] border rounded-[5px] flex flex-col justify-center text-[12px] "
                          onChange={(e) => {handleYesNo(e, index); handleInputChange(question.qno);}}
                          value={yesNoValues[index] || ""}
                        > 
                        <option value={null}>Select</option> 
                          {yesNoOptions.map((options, index) => {
                            return <option key={options} value={options}>{options}</option>;
                          })}
                        </select>{" "}
            ...
            {sortedQuestions["Matching Heading"]?.length > 0 && (
              <div className="sentence w-full bg-[#F6F8F9] h-fit py-10 mt-[18px] ">
                ...
                <p>1. {sortedQuestions["Matching Heading"][0].option1}</p> 
                 <p>2. {sortedQuestions["Matching Heading"][0].option2}</p> 
                 <p>3. {sortedQuestions["Matching Heading"][0].option3}</p> 
                 <p>4. {sortedQuestions["Matching Heading"][0].option4}</p> 
                 <p>5. {sortedQuestions["Matching Heading"][0].option5}</p> 
                </div>
                {sortedQuestions["Matching Heading"].map((question, index) => {
                  return (
                    <div className="mt-[6px] md:ml-16 ml-10">
                      <div className="flex text-[18px] font-normal  ">
                        ...
                        <select
                          ...
                          id={`heading-${index}`}
                          className=" w-[60px] h-[20px] border rounded-[5px] flex flex-col justify-center text-[12px] "
                          onChange={(e) => {handleHeading(e, index); handleInputChange(question.qno);}}
                          value={headingValues[index] || ""}
                        >
                          <option value={null}>Select</option>
                          {headingOptions.map((options, index) => {
                            return <option key={options} value={options}>{options}</option>;
                          })}
                        </select>{" "}
            ...
            <div className="my-[46px] flex flex-col items-center ">
              <p className="text-[14px] font-medium text-[#303030] ">End of Test</p>
              <p className="text-[12px] font-light text-[#303030] mt-[12px]">Submit the test to view your score and solution</p>
              <div onClick={()=>setDeleteOpen(true)} className="w-[120px] h-[35px] cursor-pointer bg-[#2B2D42] text-white text-[16px] font-normal mt-[15px] flex justify-center items-center">Submit <Upload size={14} className="ml-2" /> </div>
            </div>

           </div>
         </div>
          <div className='w-screen h-fit py-4 border-t border-b border-t-[#303030] border-b-[#303030] flex lg:flex-row flex-col justify-between px-5 items-center gap-x-2 bg-white  fixed bottom-4'>
         
            <div className="timerNdStatus lg:w-[60%] lg:justify-between flex items-center ">
              <div className="lg:w-[250px] md:w-[200px] w-[140px] h-[50px] lg:h-[67px] bg-[#2B2D42] flex justify-center items-center mr-4 lg:mr-0 ">
                <p className="font-bold text-[18px] md:text-[24px] text-white ">{minutes < 10 ? '0' + minutes : minutes}&nbsp;: &nbsp;{seconds < 10 ? '0' + seconds : seconds}</p>
              </div>

          
            <div className="flex flex-col">
              <p className="font-semibold md:text-[18px] text-[14px] text-center text-[#303030] ">
              Question status
              </p> 
              {renderQuestionStatus()}

             <div className="flex md:mt-[25px] mt-[18px] justify-center">
              <div className="md:w-[20px] md:h-[20px] w-[16px] h-[16px] rounded-full bg-[#FF467A] ml-[10px] "></div>{" "}
              <p className="font-medium md:text-[14px] text-[12px] text-[#303030] ml-[8px] ">
                Answered
              </p>
              <div className="md:w-[20px] md:h-[20px] w-[16px] h-[16px] rounded-full bg-[#E4E4E4] ml-5"></div>{" "}
              <p className="font-medium md:text-[14px] text-[12px] text-[#303030] ml-[8px] ">
                Unanswered
              </p>
             </div>
            ...
            <div className="flex lg:flex-col flex-row md:gap-x-5 gap-x-2">
               <div onClick={()=>setDeleteOpen(true)}  className="lg:w-[250px] md:w-[200px] md:h-[50px] w-[120px] h-[40px] cursor-pointer bg-[#2B2D42] text-white md:text-[18px] text-[14px] font-medium flex justify-center items-center mt-[12px]">
              Submit <Upload size={14} className="ml-2" />
               </div>

                <div onClick={()=>setOpenSolution(true)} className="lg:w-[250px] md:w-[200px] md:h-[50px] w-[120px] h-[40px] border border-[#2B2D42] cursor-pointer text-[#2B2D42] md:text-[18px] text-[14px] font-medium flex justify-center items-center mt-[12px]">
              Solution{" "}
              <svg
                className="ml-2"
                width="14"
              ...
              >
                ...
                <div onClick={()=>setExitOpen(true)} className="lg:w-[250px] md:w-[200px] md:h-[50px] w-[120px] h-[40px] cursor-pointer  text-[#2B2D42]  md:text-[18px] text-[14px] font-medium flex justify-center items-center mt-[12px]">
